...
export default function UserList({ initialUsers, currentUserId }: { initialUsers: UserType[]; currentUserId: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        // Editing
        const res = await updateUser(editingUser.id, username, password || undefined, role);
        if (!res.success) {
          setError(res.error || "Failed to update user");
        } else {
          setUsers(users.map(u => u.id === editingUser.id ? { ...u, username: res.user!.username, role: res.user!.role } : u));
          closeModal();
        }
      } else {
        // Creating
